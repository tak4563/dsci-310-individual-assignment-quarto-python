...
The Python programming language [@Python] (Van Rossum and Drake 2009)
...
![Horse populations for all provinces in Canada from 1906 - 1972.](../results/horse_pops_plot.png){#fig-horsePopulation width=90%}

We can see from @fig-horsePopulation that Ontario, Saskatchewan and Alberta 
...
Additionally, note that in @tbl-horses we consider the sample standard deviation of the number of horses during the same time span as @fig-horsePopulation.

![Horse populations for the province with the largest standard deviation.](../results/horse_pops_plot_largest_sd.png){#fig-horseLargest width=60%}

In @fig-horseLargest we zoom in and look at the province of `{python} largest_sd`, which had the largest spread of values in terms of standard deviation.
